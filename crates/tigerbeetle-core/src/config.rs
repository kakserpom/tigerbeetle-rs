...
impl ConfigCluster {
    ...
    // TODO(port): src/config.zig ConfigCluster.checksum() — needs vsr checksum.
}
...
pub mod configs {
    ...
    /// Upstream picks `test_min` when `builtin.is_test`; `cfg!(test)` is the Rust equivalent and
    /// is resolved at compile time, so this is a compile-time constant either way.
    pub const CURRENT: Config = if cfg!(test) { test_min() } else { default_production() };
}
