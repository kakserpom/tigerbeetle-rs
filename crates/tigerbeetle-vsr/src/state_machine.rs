...
use tigerbeetle_lsm::composite_key::U256;
...
use crate::tree::{LookupMemoryResult, Tree, TreeSpec};
...
/// Range-scan one transfers-groove derived index for the transfer timestamps
/// whose composite keys `(field, timestamp)` fall in the inclusive
/// `[key_min, key_max]` range (see
/// [`crate::groove::scan_composite_key_range`]).
#[must_use]
fn scan_index_timestamps<S, K, V>(
    tree: &Tree<S>,
    grid: &mut Grid,
...
where
    ...
    V: Copy,
{
    crate::groove::scan_composite_key_range(tree, grid, storage, snapshot, key_min, key_max)
        .iter()
        .map(timestamp_of)
        .collect()
}
...
impl StateMachine {
    ...
    pub fn execute_with_storage(
        ...
        match operation {
            Operation::PULSE | Operation::STATE_MACHINE_PULSE => {
                ...
                self.expire_pending_transfers_with_storage(timestamp, storage);
                Vec::new()
            }
            ...
            Operation::GET_ACCOUNT_TRANSFERS => {
                ...
                self.get_account_transfers_with_storage(&filter, storage)
            }
    ...
    /// transfers are filtered in memory. The mounted + storage-threaded path
    /// resolves the same transfers through those derived-index scans instead
    /// (see [`Self::get_account_transfers_with_storage`]) — byte identical.
    #[must_use]
    ...
    /// Execute a `get_account_transfers` query and return the reply body,
    /// resolving the transfer list through the mounted forest's transfers
    /// groove when `storage` is supplied (see [`Self::execute_with_storage`]
    /// for the threaded-storage semantics).
    ///
    /// The transfers are enumerated by prefix scans of the groove's derived
    /// index trees — upstream `get_scan_from_account_filter`
    /// (`state_machine.zig:1737-1880`): the `debit_account_id`/`credit_account_id`
    /// scans are OR-unioned per `flags.debits`/`flags.credits`, and each
    /// non-zero `user_data_*`/`code` filter AND-intersects its own index scan.
    ...
    #[must_use]
    pub fn get_account_transfers_with_storage(
        &mut self,
        filter: &AccountFilter,
        storage: Option<&mut dyn Storage>,
    ) -> Vec<u8> {
    ...
    fn persist_post_void(&mut self, event: Transfer, amount_actual: u128) {
        ...
        // force a rescan, `state_machine.zig:4226-4230`). No storage reaches
        // this path (it is moved into `create_transfers_prefetch`), so the
        // index scan stays a pure memory-table scan.
        self.update_pulse_next_timestamp(None);
    }
    ...
    /// When a forest is mounted the value is re-derived from the `expires_at`
    /// index tree's scan instead — the full scan (memory + disk) when a backing
    /// `storage` is threaded through, else the memory-table scan while the index
    /// is un-flushed — asserted equal to the recompute. A `Pending` transfer
    /// with `timeout == 0` (allowed upstream) never derives an `expires_at`
    /// entry, so it is skipped exactly as the upstream scan skips it.
    fn update_pulse_next_timestamp(&mut self, storage: Option<&mut dyn Storage>) {
        ...
        // When a forest is mounted and a backing `storage` is threaded through,
        // the next expiry comes from the `expires_at` index tree itself — the
        // derived-index scan the upstream pulse steps over, spanning the
        // in-memory tables and every visible disk table — asserted equal to the
        // authoritative pending-store recompute (upstream's
        // `expire_pending_transfers.finish`, state_machine.zig:4984-4996).
        // Without storage, the index is only consultable while wholly in memory
        // (`manifest_table_count() == 0`); otherwise (unmounted, or the index
        // was compacted to disk with no store to read it) the recompute above
        // is authoritative.
        let by_index = match (self.forest.as_mut(), storage) {
            (Some(forest), Some(storage)) => {
                let scan =
                    forest.transfers.scan_expires_at(&mut forest.grid, storage, SNAPSHOT_LATEST);
                assert_eq!(
                    scan, by_map,
            ...
            (Some(forest), None) if forest.transfers.expires_at.manifest_table_count() == 0 => {
    ...
    /// is never due.
    ///
    /// This is a thin delegate to [`Self::expire_pending_transfers_with_storage`]
    /// with no backing storage (the standalone test path).
    pub fn expire_pending_transfers(&mut self, timestamp: u64) {
        self.expire_pending_transfers_with_storage(timestamp, None);
    }

    /// [`Self::expire_pending_transfers`] with the forest's backing `storage`
    /// threaded through, so the due set can be cross-checked against the
    /// `expires_at` index scan even once the index is flushed to disk.
    ///
    ...
    /// pending is due. When a forest is mounted the due set comes from the
    /// index tree's scan — the full scan (memory + disk) when a backing
    /// `storage` is threaded through, else the memory-table scan while the
    /// index is un-flushed — asserted equal to the recompute. The forest's
    /// `expires_at` mirror entries are reaped on expiry/post/void exactly where
    /// upstream removes them (`state_machine.zig:4220` and `:4606-4612`), so
    /// the index tree stays a faithful scan source when the forest lands. This
    /// operation produces no reply body.
    pub fn expire_pending_transfers_with_storage(
        &mut self,
        timestamp: u64,
        ...
        // When a forest is mounted, the due set comes from the index tree
        // itself — the derived-index scan upstream's expiry pump steps over —
        // resolved to pending ids via the timestamp index, and is proved equal
        // to the authoritative pending-store recompute. The full scan (memory +
        // disk) runs when a backing `storage` is threaded through (so the index
        // is consultable even once flushed to disk); without storage only a
        // wholly-in-memory index can be read. Otherwise the recompute above is
        // the driver (unmounted, or a disk index with no store to read it).
        let due: Vec<(u64, u64, u128)> = match (self.forest.as_mut(), storage.as_deref_mut()) {
            (Some(forest), Some(storage)) => {
                let scan =
                    forest.transfers.scan_expires_at(&mut forest.grid, storage, SNAPSHOT_LATEST);
                let due_index: Vec<(u64, u64, u128)> = scan
                    .into_iter()
        ...
        self.update_pulse_next_timestamp(storage);
    }
...
mod tests {
    ...
    /// Expiry and pulse scheduling keep consulting the mounted forest's
    /// `expires_at` index tree after its entries are compacted to real level-0
    /// disk tables: the full `scan_expires_at` (memory + disk) feeds the same
    /// parity assert the un-flushed memory-table scan does, and a reaped entry's
    /// mutable-table tombstone suppresses its older disk-table live value
    /// (upstream's per-key stream precedence: mutable > immutable > L0 > …).
    #[test]
    #[allow(clippy::too_many_lines)]
    fn expiry_and_pulse_run_from_the_flushed_expires_at_index() {
        let mut sm = StateMachine::default();
        let mut forest = Forest::init(test_superblock(), forest_grid_options(), 32);
        let mut storage = forest_storage();
        open_forest(&mut forest, &mut storage);
        ...
        // ts 23..24: p3 (timeout 2s) and p4 (timeout 1s). p4 expires first.
        let _ = sm.create_transfers_with_storage(
            &[
                Transfer { id: 300, flags: TransferFlags::PENDING, timeout: 2, ..t(1, 2, 30) },
                Transfer { id: 400, flags: TransferFlags::PENDING, timeout: 1, ..t(1, 2, 40) },
        ...
        // Flush the index to real level-0 disk tables (bar 1's last beat swaps
        // mutable → immutable, bar 2's level-0 driver writes it), the same
        // cadence `reopened_forest_resolves_execution_from_durable_disk` uses.
        let bar_ops = constants::LSM_COMPACTION_OPS as u64;
        let bar_start = compaction::HALF_BAR_BEAT_COUNT as u64 * 2;
        let mut expires_at_flushed = false;
        for _bar in 0..2 {
            for op in bar_start..bar_start + bar_ops {
                sm.compact(op, Some(&mut storage));
            }
            if sm.forest.as_ref().expect("forest").transfers.expires_at.manifest_table_count() >= 1
            {
                expires_at_flushed = true;
            }
        }
        assert!(expires_at_flushed, "the expires_at index must flush to disk");

        // The full scan (memory + every visible disk table) yields the live
        // index: the mutable table is empty after the flush, so both entries
        // come off disk.
        assert_eq!(
            scan_flushed_expires_at(&mut sm, &mut storage),
            vec![(p4_expires, 24), (p3_expires, 23)]
        );

        // A storage-threaded no-op pulse (nothing due yet) is a "scan owed"
        // re-arm: it cross-checks the flushed index scan against the recompute
        // and points the pulse at p4's expiry.
        let _ = sm.execute_with_storage(Operation::PULSE, p4_expires - 1, &[], Some(&mut storage));
        assert_eq!(sm.pulse_next_timestamp(), p4_expires);

        // Expire p4 at its exact expiry through the storage-threaded pulse arm;
        // the due set is driven by the flushed-index scan and reaps p4's disk
        // entry. The pulse re-arms at p3's expiry.
        let _ = sm.execute_with_storage(Operation::PULSE, p4_expires, &[], Some(&mut storage));
        assert_eq!(sm.pulse_next_timestamp(), p3_expires);
        assert_eq!(
            scan_flushed_expires_at(&mut sm, &mut storage),
            vec![(p3_expires, 23)],
            "the mutable tombstone must suppress the reaped p4's disk-table entry"
        );

        // Expire p3 too; nothing remains and the pulse parks at `timestamp_max`.
        let _ = sm.execute_with_storage(Operation::PULSE, p3_expires, &[], Some(&mut storage));
        assert_eq!(sm.pulse_next_timestamp(), TimestampRange::TIMESTAMP_MAX);
        assert!(scan_flushed_expires_at(&mut sm, &mut storage).is_empty());

        audit_mirror(&mut sm);
    }

    /// Full `expires_at` index scan (memory + visible disk tables at
    /// `SNAPSHOT_LATEST`) over a mounted state machine.
    fn scan_flushed_expires_at(
        sm: &mut StateMachine,
    ...
    /// Encode an `AccountFilter` as a `GET_ACCOUNT_TRANSFERS`/
    /// `GET_ACCOUNT_BALANCES` request body (mirroring
    /// [`bytes_to_account_filter`]).
    fn account_filter_bytes(filter: &AccountFilter) -> Vec<u8> {
        let mut body = Vec::with_capacity(128);
        body.extend_from_slice(&filter.account_id.to_le_bytes());
    ...
    #[test]
    fn get_account_transfers_resolves_through_mounted_groove() {
        let mut sm = StateMachine::default();
        // The object resolutions exercise the storage path (index + value block
        // reads) but the objects tree caches the resolved blocks, so the
        // subsequent scans hit the grid cache — covering both the memory-table
        // wins and the L0/L1 disk scans.
        let mut forest = Forest::init(test_superblock(), forest_grid_options(), 32);
        let mut storage = forest_storage();
        open_forest(&mut forest, &mut storage);
        sm.mount_forest(forest);
        ...
        // a: dr=1, cr=2, code=7, user_data_64=99 (20)
        // b: dr=1, cr=2, code=8, user_data_128=500 (21)
        let mut a = t(1, 2, 10);
        a.code = 7;
        a.user_data_64 = 99;
        let mut b = t(1, 2, 11);
        ...
        let mut ts = |filter: &AccountFilter| -> Vec<u64> {
            ...
            assert_eq!(
                body,
                sm.execute(Operation::GET_ACCOUNT_TRANSFERS, 0, &account_filter_bytes(filter)),
                "groove get_account_transfers byte-identical to the committed store"
            );
            bytes_to_transfer_batch(&body)
    ...
    fn get_account_transfers_resolves_through_mounted_groove_from_durable_disk() {
        ...
        for _ in 0..1000 {
            forest_b.poll(&mut storage);
            if forest_b.transfers.debit_account_id.is_opened() && forest_b.is_idle() {
                done = true;
                break;
            }
        }
        assert!(done, "reopen must complete");
        assert!(
            forest_b.transfers.debit_account_id.manifest_table_count() >= 1,
            "the reopened forest restores the flushed L0 debit index table"
        );
        sm.forest = Some(Box::new(forest_b));

        // ── Disk tables (cold cache: scans + object resolutions read from
        // storage; blocked on the objects tree too).
        let mut filter_ts = |filter: &AccountFilter| -> Vec<u64> {
            let body = sm.execute_with_storage(
                Operation::GET_ACCOUNT_TRANSFERS,
                0,
                &account_filter_bytes(filter),
            ...
            assert_eq!(
                body,
                sm.execute(Operation::GET_ACCOUNT_TRANSFERS, 0, &account_filter_bytes(filter)),
                "disk-groove get_account_transfers byte-identical to the committed store"
            );
            bytes_to_transfer_batch(&body)
                .expect("valid transfer batch")
                .iter()
    ...
    fn account_status_name(status: u32) -> &'static str {
        match status {
            ...
            s if s == CreateAccountStatus::ImportedEventExpected as u32 => {
                "imported_event_expected"
            }
            s if s == CreateAccountStatus::ImportedEventNotExpected as u32 => {
                "imported_event_not_expected"
            }
            s if s == CreateAccountStatus::ImportedEventTimestampOutOfRange as u32 => {
                "imported_event_timestamp_out_of_range"
            }
            s if s == CreateAccountStatus::ImportedEventTimestampMustNotAdvance as u32 => {
    ...
    fn transfer_status_name(status: u32) -> &'static str {
        match status {
            ...
            s if s == CreateTransferStatus::ImportedEventTimestampMustNotAdvance as u32 => {
                "imported_event_timestamp_must_not_advance"
            }
            s if s == CreateTransferStatus::ImportedEventTimestampOutOfRange as u32 => {
                "imported_event_timestamp_out_of_range"
            }
            s if s == CreateTransferStatus::ImportedEventExpected as u32 => {
                "imported_event_expected"
            }
            s if s == CreateTransferStatus::ImportedEventNotExpected as u32 => {
                "imported_event_not_expected"
            }
            s if s
                == CreateTransferStatus::ImportedEventTimestampMustPostdateDebitAccount as u32 =>
...
create_transfers;
4:exists
7:accounts_must_have_the_same_ledger
create_transfers;
8:created
create_transfers;
9:pending_transfer_not_found
create_transfers;
10:pending_transfer_already_posted
create_transfers;
11:pending_transfer_not_pending
create_transfers;
12:credit_account_not_found
create_transfers;
13:id_already_failed
...
1:4:500:0:1:2:1:1:0:2
2:5:100:0:1:2:1:1:0:0
4:8:500:1:1:2:1:1:0:4
...
4:two_phase_pending:1:500:0:1:500:0:0:0:2:0:0:500:0
5:single_phase:2:100:0:1:500:100:0:0:2:0:0:500:100
8:two_phase_posted:4:500:1:1:0:600:0:0:2:0:0:0:600
create_accounts;
7:created
create_accounts;
15:imported_event_timestamp_must_not_regress
16:imported_event_timestamp_must_not_advance
create_accounts;
17:imported_event_timestamp_must_not_regress
create_accounts;
18:imported_event_timestamp_must_not_advance
create_accounts;
19:imported_event_timestamp_must_not_advance
20:imported_event_timestamp_must_not_advance
create_transfers;
12:created
create_transfers;
15:created
create_transfers;
23:imported_event_timestamp_must_not_advance
create_transfers;
16:created
create_transfers;
17:created
...
5:single_phase:2:100:0:1:500:100:0:0:2:0:0:500:100
8:two_phase_posted:4:500:1:1:0:600:0:0:2:0:0:0:600
12:single_phase:21:1:0:1:0:601:0:0:2:0:0:0:601
15:single_phase:22:1:0:1:0:602:0:0:2:0:0:0:602
16:two_phase_pending:24:1:0:1:1:602:0:0:2:0:0:1:602
17:two_phase_posted:25:1:24:1:0:603:0:0:2:0:0:0:603
create_transfers;
26:created
create_transfers;
27:created
...
333:26:1234:0:1:2:1:1:2:2
334:27:500:0:1:2:1:1:100:2
get_account_balances account=1;
4:500:0:0:0
5:500:100:0:0
8:0:600:0:0
...
16:two_phase_pending:24:1:0:1:1:602:0:0:2:0:0:1:602
17:two_phase_posted:25:1:24:1:0:603:0:0:2:0:0:0:603
26:two_phase_pending:333:1234:0:1:1234:603:0:0:2:0:0:1234:603
27:two_phase_pending:334:500:0:1:1734:603:0:0:2:0:0:1734:603
3000000057:two_phase_expired:333:1234:0:1:500:603:0:0:2:0:0:500:603
create_transfers;
3000000059:created
create_transfers;
3000000060:pending_transfer_expired
create_transfers;
3000000061:pending_transfer_has_different_amount
create_transfers;
3000000062:pending_transfer_already_posted
create_transfers;
3000000063:created
create_transfers;
3000000064:created
create_transfers;
3000000065:pending_transfer_already_voided
lookup_transfers n=7;
340:3000000059:201:334:1:2:1:1:0:4
350:3000000063:777:0:1:2:1:1:200:2
351:3000000064:777:350:1:2:1:1:0:8
get_account_balances account=1;
4:500:0:0:0
5:500:100:0:0
8:0:600:0:0
...
26:1234:603:0:0
27:1734:603:0:0
3000000059:0:804:0:0
3000000063:777:804:0:0
3000000064:0:804:0:0
get_account_balances account=2;
4:0:0:500:0
5:0:0:500:100
8:0:0:0:600
...
26:0:0:1234:603
27:0:0:1734:603
3000000059:0:0:0:804
3000000063:0:0:777:804
3000000064:0:0:0:804
...
3000000063:0:0:777:804
3000000064:0:0:0:804
3000000066:0:0:50:804
get_change_events ts=[3000000116,0] limit=15;
4000000097:two_phase_expired:360:50:0:1:0:804:0:0:2:0:0:0:804
create_accounts;
4000000099:created
create_transfers;
4000000100:created
lookup_accounts n=1;
4:4000000099:100:0:0:0:1:1:40
create_transfers;
4000000102:debit_account_already_closed
create_transfers;
4000000103:credit_account_already_closed
pulse at 5000000133;
lookup_accounts n=1;
4:4000000099:0:0:0:0:1:1:8
create_transfers;
5000000135:created
get_account_balances account=4;
4000000100:100:0:0:0
5000000135:0:10:0:0
create_transfers;
5000000137:created
create_transfers;
...
3000000064:0:804:0:0
3000000066:50:804:0:0
5000000137:0:805:0:0
5000000138:0:807:0:0
5000000139:0:810:0:0
5000000140:0:814:0:0
5000000141:0:819:0:0
...
350:3000000063:777:0:1:2:1:1:200:2
351:3000000064:777:350:1:2:1:1:0:8
360:3000000066:50:0:1:2:1:1:1:66
370:4000000100:100:0:4:2:1:1:1:66
373:5000000135:10:0:4:2:1:1:0:0
380:5000000137:1:0:1:2:1:1:0:0
381:5000000138:2:0:1:2:1:1:0:0
382:5000000139:3:0:1:2:1:1:0:0
383:5000000140:4:0:1:2:1:1:0:0
384:5000000141:5:0:1:2:1:1:0:0
385:5000000143:6:0:1:2:1:1:0:0
386:5000000144:7:0:1:2:1:1:0:0
387:5000000145:8:0:1:2:1:1:0:0
388:5000000146:9:0:1:2:1:1:0:0
389:5000000147:10:0:1:2:1:1:0:0
390:5000000148:11:0:1:2:1:1:0:0
391:5000000149:12:0:1:2:1:1:0:0
392:5000000150:13:0:1:2:1:1:0:0
393:5000000151:14:0:1:2:1:1:0:0
get_account_transfers account=1 reversed=0;
1:4:500:0:1:2:1:1:0:2
2:5:100:0:1:2:1:1:0:0
4:8:500:1:1:2:1:1:0:4
...
392:5000000150:13:0:1:2:1:1:0:0
393:5000000151:14:0:1:2:1:1:0:0
394:5000000152:15:0:1:2:1:1:0:0
395:5000000153:16:0:1:2:1:1:0:0
get_account_balances account=1;
4:500:0:0:0
5:500:100:0:0
8:0:600:0:0
...
5000000140:0:814:0:0
5000000141:0:819:0:0
5000000143:0:825:0:0
5000000144:0:832:0:0
5000000145:0:840:0:0
5000000146:0:849:0:0
5000000147:0:859:0:0
5000000148:0:870:0:0
5000000149:0:882:0:0
5000000150:0:895:0:0
5000000151:0:909:0:0
5000000152:0:924:0:0
5000000153:0:940:0:0
lookup_transfers n=29;
384:5000000141:5:0:1:2:1:1:0:0
380:5000000137:1:0:1:2:1:1:0:0
399:5000000157:20:0:1:2:1:1:0:0
398:5000000156:19:0:1:2:1:1:0:0
1:4:500:0:1:2:1:1:0:2
2:5:100:0:1:2:1:1:0:0
4:8:500:1:1:2:1:1:0:4
22:15:1:0:1:2:1:1:0:256
...
5000000161:created
create_transfers;
5000000162:linked_event_failed
5000000163:credit_account_not_found
5000000164:linked_event_failed
5000000165:linked_event_failed
create_transfers;
5000000166:linked_event_failed
5000000167:linked_event_chain_open
create_transfers;
5000000168:created
lookup_transfers n=10;
505:5000000159:30:0:1:2:1:1:0:1
506:5000000160:40:0:1:2:1:1:0:1
507:5000000161:50:0:1:2:1:1:0:0
514:5000000168:1:0:1:2:1:1:0:0
query_transfers ts=[0,0] limit=50 reversed=1;
514:5000000168:1:0:1:2:1:1:0:0
507:5000000161:50:0:1:2:1:1:0:0
506:5000000160:40:0:1:2:1:1:0:1
505:5000000159:30:0:1:2:1:1:0:1
399:5000000157:20:0:1:2:1:1:0:0
398:5000000156:19:0:1:2:1:1:0:0
397:5000000155:18:0:1:2:1:1:0:0
396:5000000154:17:0:1:2:1:1:0:0
395:5000000153:16:0:1:2:1:1:0:0
394:5000000152:15:0:1:2:1:1:0:0
393:5000000151:14:0:1:2:1:1:0:0
392:5000000150:13:0:1:2:1:1:0:0
...
351:3000000064:777:350:1:2:1:1:0:8
350:3000000063:777:0:1:2:1:1:200:2
get_account_transfers account=1 reversed=1;
514:5000000168:1:0:1:2:1:1:0:0
507:5000000161:50:0:1:2:1:1:0:0
506:5000000160:40:0:1:2:1:1:0:1
505:5000000159:30:0:1:2:1:1:0:1
...
600:5000000199:1:0:1:2:1:5:0:0
...
702:5000000208:1:0:700:701:7:5:0:0
...
601:5000000201:1:0:1:2:1:7:0:0
602:5000000202:2:0:1:2:1:7:0:0
603:5000000203:3:0:1:2:1:7:0:0
604:5000000204:4:0:1:2:1:7:0:0
605:5000000205:5:0:1:2:1:7:0:0
query_accounts u128=0 u64=0 u32=0 ledger=7 code=0 ts=[0,0] limit=100 reversed=0;
700:5000000206:0:1:0:0:7:70:0
701:5000000207:0:0:0:1:7:71:0
...
5000000199:single_phase:600:1:0:1:0:1136:0:0:2:0:0:0:1146
5000000201:single_phase:601:1:0:1:0:1137:0:0:2:0:0:0:1147
5000000202:single_phase:602:2:0:1:0:1139:0:0:2:0:0:0:1149
5000000203:single_phase:603:3:0:1:0:1142:0:0:2:0:0:0:1152
5000000204:single_phase:604:4:0:1:0:1146:0:0:2:0:0:0:1156
5000000205:single_phase:605:5:0:1:0:1151:0:0:2:0:0:0:1161
5000000208:single_phase:702:1:0:700:0:1:0:0:701:0:0:0:1
create_transfers;
5000000210:created
5000000211:created
create_transfers;
...
5000000216:created
create_transfers;
5000000217:linked_event_failed
5000000218:linked_event_failed
5000000219:credit_account_not_found
create_transfers;
5000000220:pending_transfer_already_posted
create_transfers;
5000000221:pending_transfer_not_found
create_transfers;
5000000222:pending_transfer_already_posted
create_transfers;
5000000223:id_already_failed
lookup_transfers n=12;
950:5000000210:100:0:1:2:1:1:0:2
951:5000000211:100:950:1:2:1:1:0:4
952:5000000212:200:0:1:2:1:1:0:2
953:5000000213:200:952:1:2:1:1:0:8
954:5000000214:50:0:1:2:1:1:0:3
955:5000000215:50:954:1:2:1:1:0:5
956:5000000216:1:0:1:2:1:1:0:0
get_account_balances account=1;
4:500:0:0:0
5:500:100:0:0
8:0:600:0:0
...
5000000152:0:924:0:0
5000000153:0:940:0:0
get_change_events ts=[5000000210,0] limit=100;
5000000210:two_phase_pending:950:100:0:1:100:1151:0:0:2:0:0:100:1161
5000000211:two_phase_posted:951:100:950:1:0:1251:0:0:2:0:0:0:1261
5000000212:two_phase_pending:952:200:0:1:200:1251:0:0:2:0:0:200:1261
5000000213:two_phase_voided:953:200:952:1:0:1251:0:0:2:0:0:0:1261
5000000214:two_phase_pending:954:50:0:1:50:1251:0:0:2:0:0:50:1261
5000000215:two_phase_posted:955:50:954:1:0:1301:0:0:2:0:0:0:1311
5000000216:single_phase:956:1:0:1:0:1302:0:0:2:0:0:0:1312
";
    ...
    fn accounting_matches_upstream_golden() {
        ...
        for row in sm.create_accounts(&[a1, a2], 2).as_chunks::<16>().0 {
        ...
        out.push_str("create_accounts;\n");
        for row in sm.create_accounts(&[a3], 3).as_chunks::<16>().0 {
            let ts = u64::from_le_bytes(row[0..8].try_into().expect("8-byte slice"));
            let status = u32::from_le_bytes(row[8..12].try_into().expect("4-byte slice"));
            let _ = writeln!(out, "{ts}:{}", account_status_name(status));
        }
        ...
        bulk(&mut sm, &mut out, &[t1], 4);
        bulk(&mut sm, &mut out, &[t2], 5);
        // Duplicate of t1 (`.exists`) and a ledger-mismatched transfer
        // (`.accounts_must_have_the_same_ledger`).
        bulk(&mut sm, &mut out, &[t1, t3], 7);
        ...
        bulk(&mut sm, &mut out, &[orphan_fail], 12);
        bulk(&mut sm, &mut out, &[orphan_fail], 13);
        ...
        // regression.
        accounts_batch(&mut sm, &mut out, &[imported_account(33, 7)], 14);
        // Imported accounts stored at their own timestamps.
        accounts_batch(&mut sm, &mut out, &[imported_account(10, 6), imported_account(20, 30)], 16);
        // Retry: idempotency precedes the regression check (reports `.exists`
        // at the stored timestamp even when the imported one differs).
        accounts_batch(&mut sm, &mut out, &[imported_account(10, 6)], 17);
        accounts_batch(&mut sm, &mut out, &[imported_account(20, 29)], 18);
        ...
        // the CDC records it.
        bulk(&mut sm, &mut out, &[Transfer { timeout: 2, ..pending_never(333, 1234) }], 26);
        bulk(&mut sm, &mut out, &[Transfer { timeout: 100, ..pending_never(334, 500) }], 27);

        out.push_str("pulse at 3000000057;\n");
        let reply = sm.execute(Operation::STATE_MACHINE_PULSE, 3_000_000_057, &[]);
        assert!(reply.is_empty());
        ...
        bulk(&mut sm, &mut out, &[posting(340, 201, 334)], 3_000_000_059);
        bulk(&mut sm, &mut out, &[posting(341, 1234, 333)], 3_000_000_060);
        bulk(&mut sm, &mut out, &[voiding(342, 300, 334)], 3_000_000_061);
        bulk(&mut sm, &mut out, &[posting(344, 1, 334)], 3_000_000_062);
        bulk(
            &mut sm,
            &mut out,
        ...
        bulk(&mut sm, &mut out, &[voiding(351, 777, 350)], 3_000_000_064);
        bulk(&mut sm, &mut out, &[voiding(352, 777, 350)], 3_000_000_065);
        ...
        out.push_str("pulse at 4000000097;\n");
        let reply = sm.execute(Operation::STATE_MACHINE_PULSE, 4_000_000_097, &[]);
        assert!(reply.is_empty());
        out.push_str("lookup_accounts n=1;\n");
        for acc in bytes_to_account_batch(&sm.lookup_accounts(&[1])).expect("valid account batch") {
            let _ = writeln!(out, "{}", format_account(&acc));
        ...
        out.push_str("create_accounts;\n");
        for row in sm.create_accounts(&[a(4, 1, 1)], 4_000_000_099).as_chunks::<16>().0 {
            let ts = u64::from_le_bytes(row[0..8].try_into().expect("8-byte slice"));
            let status = u32::from_le_bytes(row[8..12].try_into().expect("4-byte slice"));
            let _ = writeln!(out, "{ts}:{}", account_status_name(status));
        }
        ...
        out.push_str("pulse at 5000000133;\n");
        let reply = sm.execute(Operation::STATE_MACHINE_PULSE, 5_000_000_133, &[]);
        assert!(reply.is_empty());
        out.push_str("lookup_accounts n=1;\n");
        for acc in bytes_to_account_batch(&sm.lookup_accounts(&[4])).expect("valid account batch") {
            let _ = writeln!(out, "{}", format_account(&acc));
        ...
        bulk(&mut sm, &mut out, &[debit1(380, 1)], 5_000_000_137);
        bulk(&mut sm, &mut out, &[debit1(381, 2)], 5_000_000_138);
        bulk(&mut sm, &mut out, &[debit1(382, 3)], 5_000_000_139);
        bulk(&mut sm, &mut out, &[debit1(383, 4)], 5_000_000_140);
        bulk(&mut sm, &mut out, &[debit1(384, 5)], 5_000_000_141);
        account_balances(&mut sm, &mut out, 1);
        let filter = AccountFilter {
            account_id: 1,
        ...
        bulk(&mut sm, &mut out, &[debit1(385, 6)], 5_000_000_143);
        bulk(&mut sm, &mut out, &[debit1(386, 7)], 5_000_000_144);
        bulk(&mut sm, &mut out, &[debit1(387, 8)], 5_000_000_145);
        bulk(&mut sm, &mut out, &[debit1(388, 9)], 5_000_000_146);
        bulk(&mut sm, &mut out, &[debit1(389, 10)], 5_000_000_147);
        bulk(&mut sm, &mut out, &[debit1(390, 11)], 5_000_000_148);
        bulk(&mut sm, &mut out, &[debit1(391, 12)], 5_000_000_149);
        bulk(&mut sm, &mut out, &[debit1(392, 13)], 5_000_000_150);
        bulk(&mut sm, &mut out, &[debit1(393, 14)], 5_000_000_151);
        bulk(&mut sm, &mut out, &[debit1(394, 15)], 5_000_000_152);
        bulk(&mut sm, &mut out, &[debit1(395, 16)], 5_000_000_153);
        bulk(&mut sm, &mut out, &[debit1(396, 17)], 5_000_000_154);
        bulk(&mut sm, &mut out, &[debit1(397, 18)], 5_000_000_155);
        bulk(&mut sm, &mut out, &[debit1(398, 19)], 5_000_000_156);
        bulk(&mut sm, &mut out, &[debit1(399, 20)], 5_000_000_157);
        ...
        // created.
        bulk(&mut sm, &mut out, &[chain1(512, 90, true), chain1(513, 100, true)], 5_000_000_167);
        bulk(&mut sm, &mut out, &[chain1(514, 1, false)], 5_000_000_168);
        ...
        // Reversed scans at the reply cap: `reversed` flips the scan to
        // descending and the multi-batch cap (29) truncates to the newest 29
        // rows — pinning sort-then-cap on both multi-batch reads.
        let q = query(0, 0, 50, true);
        let _ = writeln!(
            out,
            "query_transfers ts=[{},{}] limit={} reversed={};",
            q.timestamp_min,
        ...
        let _ = writeln!(
            out,
            "get_account_transfers account={} reversed={};",
            filter.account_id,
        ...
        many.push(a(901, 1, 1));
        accounts_batch(&mut sm, &mut out, &many, 5_000_000_198);
        ...
        bulk(&mut sm, &mut out, &[pending(952, 200, false), void(953, 952)], 5_000_000_213);
        ...
        bulk(&mut sm, &mut out, &[post(960, 50, 954, false)], 5_000_000_220);
        bulk(&mut sm, &mut out, &[post(961, 300, 957, false)], 5_000_000_221);
        bulk(&mut sm, &mut out, &[post(960, 50, 954, false)], 5_000_000_222);
        bulk(&mut sm, &mut out, &[post(961, 300, 957, false)], 5_000_000_223);
        ...
        // The 950-956 run's CDC stream, windowed to the post/void section
        // (the balance dump above is reply-capped and drops these rows):
        // pins the in-batch post 951@211, the `two_phase_voided` for the
        // in-batch void 953@213 of pending 952@212, and the closed chain
        // 954-956.
        let filter = ChangeEventsFilter {
            timestamp_min: 5_000_000_210,
            timestamp_max: 0,
            limit: 100,
            reserved: [0; 44],
        };
